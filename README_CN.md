...
```text
消息进入
  └─ agent_end hook：保存本轮新消息到 gm_messages，并触发提取
...
Graph Memory 现在是 hook-only 插件：不再注册 ContextEngine，不占用 `plugins.slots.contextEngine`。OpenClaw 当前选中的 ContextEngine/runtime 继续负责消息组装和真正的上下文压缩；Graph Memory 只做记忆侧副作用：召回、提取、图维护和 `gm_*` 工具。

## 运行入口

graph-memory 通过运行时 hooks 和 tools 接入 OpenClaw：

- `api.on("before_prompt_build", ...)` 执行召回和 KG 上下文注入。
- `api.on("before_message_write", ...)` 支持 `autoRecallMode=index`，只给匹配的 user message 写入短 recall index；不会在这里清理 transcript 里的瞬时 `<gm_memory>`。
- `api.on("agent_end", ...)` 保存完整 turn 的新消息并触发提取/周期维护。
- `api.on("before_compaction", ...)` / `api.on("after_compaction", ...)` 做压缩前后记忆侧处理。
- `api.on("subagent_spawned", ...)` / `api.on("subagent_ended", ...)` 维护子 agent 相关内存态。
- `api.on("session_end", ...)` 执行 finalize 和后台维护。
- `api.registerTool(...)` 暴露 `gm_*` 工具。
...
| `before_message_write` | 消息写入前 | index 模式写入短 recall index；这里刻意不清理 transcript 里的瞬时 `<gm_memory>` | await |
| `before_compaction` / `after_compaction` | runtime 压缩前后 | 兜底提取、标记 compact active nodes；真正压缩由 OpenClaw runtime/当前 CE 负责 | await，提取/归纳尽量后台化 |
| `agent_end` | 每轮 assistant 回复后 | 保存新消息、触发提取、周期 topic/session 归纳、轻量 PageRank | 提取/topic induction 为 fire-and-forget；周期 session induction 当前会 await |
| `subagent_spawned` | 创建 subagent 后 | 尽力将父 session 当前召回图谱复制给子 session | await |
| `subagent_ended` | 子 session 结束 | 清理子 session recalled/counter 内存态 | await |
...
| runtime lifecycle cleanup | 插件卸载/重载/禁用 | 清理内存 map | await |
...
原始聊天上下文仍由 OpenClaw runtime/当前 ContextEngine 正常压缩。graph-memory 不接管完整压缩流程。通过 compaction hooks 它会：
...
| `compactTurnCount` | `6` | agent_end 周期性归纳/轻量维护间隔 |
