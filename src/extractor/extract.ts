...
5. 置信度更新（beliefUpdates）：
   **重要**：beliefUpdates 只针对知识图谱 XML 中已存在的节点，不得为本轮新创建的节点输出 beliefUpdates。
   5.1 判断逻辑：逐一扫描知识图谱 XML 中的已有节点，判断本轮对话是否对该节点提供了明确的验证证据（支持或否定），历史对话中的内容已做过置信度更新，不判断历史对话内容对已有节点的支持与否定
   5.2 只在有明确证据时输出，无证据则 beliefUpdates 为空数组 []
   5.3 supported（支持/正例）— 本轮对话证明该节点的内容是正确的：
       - 对话中 Agent 按照该节点的知识/指导执行操作，最终成功（用户确认或工具返回成功）
       - 对话中 Agent 引用了该节点的内容做出判断，用户没有纠正
       - 对话中的实际结果与该节点的预期一致
   5.4 contradicted（反对/反例）— 本轮对话证明该节点的内容有误或过时：
       - 用户明确说该节点内容有误（如"不对"、"错了"、"这个规则太严格"）
       - Agent 按照该节点指导执行但失败，且失败原因是节点信息本身有误（非外部因素）
       - 对话中的实际结果与该节点的预期矛盾
   5.5 每个置信度更新必须包含 4 个字段：
...
6. 顾问建议（advisorySuggestions）—— 仅针对本轮新建节点：
   6.1 advisorySuggestions 是可选的。没有符合以下条件的新建节点时返回空数组 []。
   6.2 触发条件（同时满足）：
       - 新建节点的内容长度超过 500 字，且为结构化数据（配置、数据库 schema、完整的操作步骤序列等）
       - 或者是需要跨 session 精确复现的复杂配置（如完整的 NixOS flake 配置、数据库建表语句、完整的 Docker compose 文件等）
       - 注意：普通知识点（简短说明、概念解释）、简单的单条命令或单点知识，不需要写成文档
   6.3 每条建议必须包含 nodeName（精确匹配本轮新建节点的 name）。

7. 输出规范：
   7.1 只返回 JSON，格式为 {"nodes":[...],"edges":[...],"beliefUpdates":[...]},禁止 markdown 代码块包裹
   7.2 禁止解释文字，禁止额外字段
   7.3 没有知识产出且无需置信度更新时返回 {"nodes":[],"edges":[]}
   7.4 有置信度更新但无新知识时返回 {"nodes":[],"edges":[],"beliefUpdates":[...]}
   7.5 每条 edge 的 description 必须写具体内容，不能为空或"见上文"
   7.6 beliefUpdates 只针对已有节点，新创建的节点不需要 beliefUpdates
   7.7 advisorySuggestions 仅有建议时返回 {"nodes":[...],"edges":[...],"advisorySuggestions":[...]}
   7.8 所有字段都有时返回 {"nodes":[...],"edges":[...],"beliefUpdates":[...],"advisorySuggestions":[...]}
   7.9 advisorySuggestions 字段不存在时等价于空数组

7. 历史对话与当前对话的处理原则:
   7.1 对话分为"=== 历史对话 ==="(同 session 内更早的已提取消息)和"=== 当前对话 ==="(待提取的新消息)
   7.2 **优先为当前对话提取知识**,当前对话中的 TASK / SKILL / EVENT / KNOWLEDGE / STATUS 必须完整提取
   7.3 历史对话作为上下文参考,如果发现其中有重要知识被遗漏(当前对话中有关联引用或明确讨论),也可以提取
   7.4 历史对话中的敏感信息(密码、API Key 等)请同样套用脱敏规则

8. 敏感信息保护(所有节点类型强制执行):
   8.1 content 和 description 中,禁止写入任何实际敏感值(密码、API Key、Token、Access Token、Secret、Credentials 等)
   8.2 如需记录凭证,只写获取方式,例如:
       · API Key: 从 rbw 获取(rbw get xxx)
       · 密码:用户在对话中提供
       · Token: 环境变量 $OPENAI_API_KEY
       · Credentials: 从飞书 OAuth 获取
   8.3 凭证描述要具体到足以让未来重新获取,但不得包含实际值`;
