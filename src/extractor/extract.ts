...
2. 关系提取（边）：
   ...
   2.3 name（边类型名）建议从以下15种边类型中选择，若两个节点确实有现有边类型不能概括的语义关系，允许自定义边名：
     关系型：依赖、扩展、冲突、互补
     行为型：使用、执行、替代、触发、导致
     认知型：发现、揭示、引用、分析
     问题型：遇到、解决、修正、验证
...
2. 补充遗漏关系：
   ...
   边类型 name 建议从以下15种中选择：关系型（依赖、扩展、冲突、互补）、行为型（使用、执行、替代、触发、导致）、认知型（发现、揭示、引用、分析）、问题型（遇到、解决、修正、验证）、产出型（产出、实现、定义）。若两个节点确实有现有边类型不能概括的语义关系，允许自定义边名。
   写入 newEdges 数组。
