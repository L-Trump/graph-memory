...
2. 关系提取（边）：
   ...
   2.3 name（边类型名）建议从以下20种边类型中选择，若两个节点确实有现有边类型不能概括的语义关系，允许自定义边名：
     关系型：依赖、扩展、冲突、互补
     行为型：使用、执行、替代、触发、导致
     认知型：发现、揭示、引用、分析、理解
     问题型：遇到、解决、修正、验证、修复
   ...
   2.5 优先为新节点建边，同时也鼓励发现已有L1层节点和已有节点之间遗漏的关联（L1层节点之间或L1层节点和其他节点间的新建边也可写入 edges）
...
2. 补充遗漏关系：
   ...
   边类型 name 建议从以下20种中选择：关系型（依赖、扩展、冲突、互补）、行为型（使用、执行、替代、触发、导致）、认知型（发现、揭示、引用、分析、理解）、问题型（遇到、解决、修正、验证、修复）、产出型（产出、实现、定义）、参考型（查阅、查看、参考）。若两个节点确实有现有边类型不能概括的语义关系，允许自定义边名。
   写入 newEdges 数组。
